...
export function epicToMarkdown(epic: EpicProposal): string {
  ...
  if (epic.inScope.length) {
    out.push('## In scope', ...epic.inScope.map((s) => `- ${s}`), '');
  }
  if (epic.outOfScope.length) {
  ...
  if (epic.openQuestions.length) {
    out.push('## Open questions', ...epic.openQuestions.map((q) => `- ${q}`), '');
  }
  out.push('---', `_Generated by ReqForge from the source PRD. Sizing: ${epic.sizing}._`);
  return out.join('\n');
}
...
export function storyToMarkdown(story: StoryProposal): string {
  ...
  out.push('');
  if (story.openQuestions.length) {
    out.push('## Open questions', ...story.openQuestions.map((q) => `- ${q}`), '');
  }
  out.push('---', `_Generated by ReqForge. Estimate: ${story.points} points._`);
  return out.join('\n');
}
